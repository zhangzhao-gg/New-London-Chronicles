/**
 * [INPUT]: `GET /api/city`、`POST /api/tasks/assign-next`、当前用户初始态、`UI/city.html`
 * [OUTPUT]: 城市页客户端壳层、30 秒轮询、贴近静态原型的 HUD 布局与 FOCUS 交互
 * [POS]: 位于 `hooks/use-city.tsx`，被 `app/city/page.tsx` 消费
...
type DistrictKey = "resource" | "residential" | "medical" | "food" | "exploration";
type DistrictStatus = "可采集" | "建造进行中" | "无进行中任务" | "资源不足";
...
export function CityPageShell({ initialUser }: { initialUser: UserDto }) {
  ...
  const totalWorkers = districts.reduce((sum, district) => sum + district.workingCount, 0);
        ...
        <section className="relative min-h-[680px] flex-1 overflow-hidden bg-slate-950 lg:min-h-[calc(100vh-129px)]">
          ...
          <div className="absolute bottom-6 left-1/2 z-10 flex w-[min(92%,1120px)] -translate-x-1/2 flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            <div className="flex flex-1 items-center gap-2 rounded-sm border border-[rgba(244,164,98,0.3)] bg-[rgba(20,13,9,0.9)] p-1 backdrop-blur-md shadow-2xl">
              ...
              <button
                className="flex flex-1 flex-col items-center justify-center py-3 text-slate-500 transition-colors hover:bg-[rgba(244,164,98,0.05)] hover:text-[var(--nlc-orange)]"
                onClick={() => void focus()}
                type="button"
              >
                <span className="text-lg leading-none">◎</span>
